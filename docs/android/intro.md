...
adsynapse Android SDK는 **GitHub Packages**를 통해 제공됩니다.
...
repositories {
   ...
   maven {  
      url = uri("https://maven.pkg.github.com/Infinit-C/adsynapse-aos-sdk")  
   }
   ...
}
```
...
dependencies {
   ...
   implementation "io.id8:adsynapse-aos-sdk:x.y.z"
   ...
}
```
...
`publisher`키는 `io.id8.android.adsynapse.PUBLISHER_ID`로 입력합니다.
`media`키는 `io.id8.android.adsynapse.APP_ID`로 입력합니다.

```xml
<manifest>
    <application>
        <meta-data
                android:name="io.id8.android.adsynapse.PUBLISHER_ID"
                android:value="i8-adc-pub-126412831823-45789723847"/>
        <meta-data
                android:name="io.id8.android.adsynapse.APP_ID"
